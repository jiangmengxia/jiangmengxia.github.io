# 事件合成 -createSyntheticEvent

使用工厂函数createSyntheticEvent创建不同类型的合成事件的实例。

合成事件的优势是：

* 跨平台兼容性
* 事件参数是经过优化的，剔除了不需要的参数
* 此外事件根据优先级及事件本身特性定义不同类型的事件（这块暂时未研究，待定）

```jsx
<div onClick={(e)=>{
    console.log(e)   // 这个e就是一个合成事件, 参数是经过优化过的。
}}> 这是一个JSX </div>
```
